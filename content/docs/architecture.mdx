...
5. **Atom**: Identity and authorization platform for entities, tenants, and access policies
...
The [Propeller Kubernetes Operator](https://github.com/absmach/propeller-k8s-operator) provides a way to deploy and manage Propeller on Kubernetes clusters. It handles the orchestration of tasks across proplets, scheduling, and resource management. The Operator is responsible for:
...
- **Tenants**: Isolated namespaces for organizing resources
- **Entities**: Identities (users, services) with attached credentials
- **Resources**: Channels and other addressable objects
- **Permissions**: Fine-grained access control via permission blocks and direct policies
- **API Keys**: Scoped credentials for service-to-service authentication

Propeller uses Atom to provision service entities (manager, proplet, proxy) with API keys, create channels as MQTT topics, and grant publish/subscribe permissions through direct policies.
...
```
m/{tenantID}/c/{channelID}/control/proplet/alive
...
7. **Interrupted**: Task interrupted during execution
...
- **Atom**: Identity, authorization, and API key management for Propeller services
