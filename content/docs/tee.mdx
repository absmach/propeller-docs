...
description: "Run WebAssembly workloads inside hardware-protected Trusted Execution Environments. Supports Intel TDX, AMD SEV-SNP, Intel SGX, IBM SE, and ARM CCA for confidential Cloud-Edge computing with Propeller."
---
...
- **IBM SE** — IBM Secure Execution for IBM Z and LinuxONE (s390x)
- **ARM CCA** — Confidential Compute Architecture with Realm Management Extension

Additional device attestation is also supported for NVIDIA GPUs, Hygon DCU accelerators, and TPM modules.
...
3. Attestation Agent retrieves attestation proof from TEE hardware
4. Key Broker Service validates attestation and releases decryption keys
5. Confidential Data Hub delivers keys to image-rs for decryption
6. Proplet decrypts the WASM image and executes it
...
- [Guest Components](https://github.com/confidential-containers/guest-components) — in-guest TEE attestation and secret management:
  - **Attestation Agent (AA)** — communicates with TEE hardware and KBS
  - **Confidential Data Hub (CDH)** — central broker for secrets, sealed data, and image resources
  - **API Server REST (ASR)** — REST HTTP gateway that forwards to AA and CDH
...
Resource URIs use the format `kbs:///<repository>/<type>/<tag>`. Plugin URIs are also supported using `kbs+<plugin>:///` to route requests through specific KBS plugins (e.g., `kbs+pkcs11:///default/key/my-key`).
...
| -------------------- | ---------------------------- | ------------------- |
| `PROPLET_TENANT_ID`  | Atom tenant identifier       | `my-tenant-123`     |
...
| `ENABLE_CVM`           | CVM mode (`auto`/`tdx`/`sev`/`snp`/`se`/`cca`/`none`) | `auto`                 |
...
# Force AMD SEV (legacy SEV without attester)
ENABLE_CVM=sev ./qemu.sh

# Force AMD SEV-SNP (with SNP attester)
ENABLE_CVM=snp ./qemu.sh

# Force IBM Secure Execution (s390x)
ENABLE_CVM=se ./qemu.sh

# Force ARM CCA (Realm Management Extension)
ENABLE_CVM=cca ./qemu.sh
...
export PROPLET_MQTT_ADDRESS="tcp://10.0.2.2:1883"
...
export PROPLET_MQTT_ADDRESS="tcp://10.0.2.2:1883"
export KBS_URL="http://10.0.2.2:8082"
export PROPLET_AA_CONFIG_PATH=/etc/attestation-agent.toml
```

The `PROPLET_AA_CONFIG_PATH` points to the Attestation Agent config (must use `.toml` or `.json` extension). The default file looks like this:
...
The `[token_configs.kbs]` section configures the KBS connection:

- `url` — KBS endpoint URL
- `cert` (optional) — PEM certificate for TLS connections to KBS
- `tee_key_algorithm` (optional) — key exchange algorithm, defaults to `ECDH-ES+A256KW-P256`

The `[eventlog_config]` section controls event log recording:

- `init_pcr` — PCR register index for init entry (default: 17)
- `enable_eventlog` — whether to record event logs (default: false)
...
The guest components stack consists of:

| Component                       | Role                                                                                                                                                                                                                                                                 |
| ------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Attestation Agent (AA)**      | Generates attestation evidence and tokens from TEE hardware. Supports Intel TDX, AMD SEV-SNP, Intel SGX, IBM SE, ARM CCA, Hygon CSV, and Azure vTPM platforms. Also supports additional device attestation for NVIDIA GPUs, Hygon DCU accelerators, and TPM modules. |
| **Confidential Data Hub (CDH)** | Central broker for secrets, sealed storage (LUKS2), KMS integration, and image resource management. Retrieves decryption keys from KBS after attestation.                                                                                                            |
| **API Server REST (ASR)**       | REST HTTP gateway that exposes AA and CDH services over HTTP. Provides endpoints for evidence, tokens, resources, and additional device attestation.                                                                                                                 |
| **CoCo Keyprovider**            | Handles OCI image encryption/decryption key wrapping. Registers keys with KBS and provides them to image-rs for decryption.                                                                                                                                          |
| **image-rs**                    | OCI container image pulling, layer decryption, and signature verification with multiple backend support.                                                                                                                                                             |

Data flow: `REST API (ASR) → ttrpc → AA (attestation) + CDH (secrets/images) → image-rs (decryption with keys from CDH)`

### Execution flow

1. Detection - Proplet checks for TEE device files at startup
2. Task receipt - Manager publishes an encrypted task request
3. Image pull - image-rs downloads the encrypted OCI image
4. Attestation - Attestation Agent generates proof of TEE environment (with optional additional device evidence)
5. Key retrieval - KBS validates attestation and releases key via Confidential Data Hub
6. Decryption - Image layers decrypted inside the TEE by image-rs
...
- Multi-device attestation - supports composite evidence from additional TEE devices (NVIDIA GPUs, Hygon DCU, TPM) alongside the primary TEE platform
