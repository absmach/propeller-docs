...
The Hardware Abstraction Layer (HAL) provides a single script — [hal/ubuntu/qemu.sh](https://github.com/absmach/propeller/blob/main/hal/ubuntu/qemu.sh) — that builds an **Ubuntu Noble (24.04)** cloud image and boots it as a QEMU virtual machine. On first boot, cloud-init compiles and installs all Propeller services inside the VM automatically.

The first boot takes **10–15 minutes** while cloud-init installs packages and compiles services from source. Subsequent boots start all services immediately in under 30 seconds.
...
1. Installs system packages (`build-essential`, `libssl-dev`, `protobuf-compiler`, `libtss2-dev`, `tpm2-tools`, `rustup`, and others)
2. Installs the Rust toolchain via `rustup`
3. Downloads and installs the latest Wasmtime release binary from GitHub
4. Clones and compiles the **Attestation Agent** from [guest-components](https://github.com/confidential-containers/guest-components) with all attesters enabled
5. Clones and compiles the **CoCo Keyprovider** for OCI image decryption
6. Clones and compiles **Proplet** from the Propeller repository
7. Verifies all four binaries exist before enabling any services
8. Enables and starts three systemd services: `attestation-agent`, `coco-keyprovider`, and `proplet`
...
| -------------------- | --------------------- |
| `PROPLET_TENANT_ID`  | Atom tenant ID        |
| `PROPLET_ENTITY_ID`  | Atom entity ID        |
| `PROPLET_API_KEY`    | Atom API key          |
| `PROPLET_CHANNEL_ID` | Atom channel ID       |
