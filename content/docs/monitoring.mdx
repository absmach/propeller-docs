...
4. Manager receives and stores metrics in the configured storage backend
...
Proplets publish two categories of metrics:

| Topic                                                       | Content                                                                                                         |
| ----------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------- |
| `m/{tenant_id}/c/{channel_id}/control/proplet/task_metrics` | Per-task process metrics (CPU, memory, disk I/O, threads, file descriptors) — published while a task is running |
| `m/{tenant_id}/c/{channel_id}/control/proplet/metrics`      | Proplet-level resource metrics (CPU, memory, cgroup stats) — published on a fixed interval                      |
...
**Subscribe to per-task metrics for a specific domain/channel:**

```bash
mosquitto_sub -h localhost -p 1883 \
  -t "m/domain-123/c/channel-456/control/proplet/task_metrics" -v
```

**Subscribe to proplet metrics:**

```bash
mosquitto_sub -h localhost -p 1883 \
  -t "m/+/c/+/control/proplet/metrics" -v
```

**Subscribe to task metrics:**

```bash
mosquitto_sub -h localhost -p 1883 \
  -t "m/+/c/+/control/proplet/task_metrics" -v
```
...
```bash
mosquitto_sub -h localhost -p 1883 -t "m/+/c/+/control/proplet/metrics" -v
mosquitto_sub -h localhost -p 1883 -t "m/+/c/+/control/proplet/task_metrics" -v
```
...
| `interval`                 | integer | Collection interval in seconds (1–120) |
