...
[00:00:02.952,000] <inf> mqtt_client: Discovery published successfully to topic: m/<tenant_id>/c/<channel_id>/control/proplet/create
...
The `PROPLET_ID` defined in `src/main.c` is used as both the MQTT **client ID** and **username** when connecting to the broker. This value must match the Atom client ID registered with the broker, otherwise the connection will be rejected.
...
```c
publish_discovery(tenant_id, PROPLET_ID, channel_id);
```
...
1. **Receive**: The Manager publishes to `m/<tenant_id>/c/<channel_id>/control/manager/start` with a JSON payload
...
4. **Report**: The result is published as a string to `m/<tenant_id>/c/<channel_id>/control/proplet/results`
...
The Manager publishes to `m/<tenant_id>/c/<channel_id>/control/manager/stop` with:
...
2. Check configuration matches broker settings
