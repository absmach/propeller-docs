...
The `CRT_FILE_NAME` determines the output: `certs/<name>.crt` and `certs/<name>.key`. Each service needs a unique client secret (UUID) — this is the client identity in Atom and must match what you set in `*_CLIENT_KEY` in `.env`.
