...
This is a Propeller Kubernetes Operator example. It provisions [Atom](https://docs.atom.absmach.eu/), deploys the operator, registers an external proplet, and runs a WebAssembly addition task.
...
## Step 4 — Start the Propeller Stack (Atom + FluxMQ + Proplet)

The operator communicates with proplets over MQTT via the Atom + FluxMQ broker stack. Start the full Propeller Docker stack (broker services + Propeller services) from the propeller repository:
...
```bash
 Container propeller-base-spicedb-db Started
 Container propeller-base-auth-redis Started
 Container propeller-base-users-db Started
 Container propeller-base-channels-db Started
 Container propeller-base-clients-db Started
 Container propeller-base-fluxmq-node1 Started
 Container propeller-base-fluxmq-node2 Started
 Container propeller-base-fluxmq-node3 Started
 Container propeller-base-fluxmq-auth Started
 Container propeller-base-nginx Started
 Container propeller-base-users Started
 Container propeller-base-channels Started
 Container propeller-base-clients Started
 Container propeller-base-auth Started
 Container propeller-base-domains Started
...
```bash
docker ps --format "table {{.Names}}\t{{.Status}}" | grep -E "propeller"
```
...
propeller-base-nginx             Up 37 seconds
propeller-base-clients           Up 35 seconds
propeller-base-channels          Up 35 seconds
propeller-base-users             Up 33 seconds
propeller-base-auth              Up 30 seconds
propeller-base-domains           Up 28 seconds
propeller-base-fluxmq-node1      Up 37 seconds
propeller-base-fluxmq-node2      Up 37 seconds
propeller-base-fluxmq-node3      Up 37 seconds
propeller-base-fluxmq-auth       Up 37 seconds
propeller-base-spicedb           Up 37 seconds
propeller-base-users-db          Up 37 seconds
propeller-base-channels-db       Up 37 seconds
propeller-base-clients-db        Up 37 seconds
propeller-base-auth-db           Up 37 seconds
propeller-base-auth-redis        Up 37 seconds
propeller-base-jaeger            Up 37 seconds
```
...
## Step 5 — Provision Atom Credentials

The operator and proplet need Atom credentials—a domain, channel, and two client IDs with keys—to authenticate on MQTT. Use `propeller-cli provision` to create all these resources through the Atom HTTP API and write the result directly to `docker/config.toml`.
...
Recreate only the propeller containers (not Atom) to pick up the new env values:
...
Confirm the manager credentials can publish on MQTT. The MQTT client ID must match the Atom client ID:
...
```json
{
  "proplet_id": "b64502f0-25bc-4926-8fbc-ad508f75d96c",
  "status": "alive",
  "namespace": "default"
}
```
